    ...
    <header class="topbar">
      <div class="site-shell nav">
        ...
        <nav class="nav-links">
          ...
          <a class="nav-link" href="/#voices">의견 보기</a>
        </nav>
      </div>
    </header>
    ${content}
...
function homeTemplate() {
  ...
  const submitText = edit ? "수정 저장" : "의견 등록";
  ...
  return layout(`
    <section class="site-shell hero protest-hero">
      <div class="hero-card manifesto">
        ...
        <p class="lead">
          우리는 학교와 사교육이 만든 과열 경쟁, 침묵을 강요하는 교실,
          진도만 밀어붙이는 수업, 체험학습을 위축시키는 책임 회피를 기록한다.
          이것은 푸념이 아니라 구조에 대한 고발이다.
        </p>
      ...
      <div class="hero-side hero-card protest-board">
        <div class="point">
          <strong>성적은 인간의 가격표가 아니다</strong>
          <span>학생을 등수와 점수로만 평가하는 문화는 배움이 아니라 분류다.</span>
        </div>
        <div class="point">
          <strong>교실은 암기 공장이 아니다</strong>
          <span>질문, 토론, 실패할 권리가 사라진 수업은 교육의 이름을 빌린 통제다.</span>
        </div>
        <div class="point">
          <strong>사교육은 선택이 아니라 압박이 되었다</strong>
          <span>불안을 팔아 경쟁을 키우는 구조를 더 이상 정상으로 부를 수 없다.</span>
        </div>
      </div>
    </section>
    ...
    <section class="site-shell section indictment">
      ...
      <div class="grid-4">
        <article class="stat-card">
          <b>01</b>
          <p>입시 경쟁을 이유로 학생의 불안과 고통을 개인 책임으로 돌리는 학교 문화를 규탄한다.</p>
        </article>
        <article class="stat-card">
          <b>02</b>
          <p>공교육의 빈틈을 사교육 비용으로 메우게 만들고, 가정 형편에 따라 출발선을 갈라놓는 구조를 규탄한다.</p>
        </article>
        <article class="stat-card">
          <b>03</b>
          <p>이해보다 속도, 사고보다 암기, 성장보다 진도를 우선하며 학생을 질문 없는 수험 기계로 만드는 수업을 규탄한다.</p>
        </article>
        <article class="stat-card">
          <b>04</b>
          <p>현장 배움과 체험학습을 행정 부담과 책임 회피 뒤로 밀어내는 분위기를 규탄한다.</p>
        </article>
        <article class="stat-card">
          <b>05</b>
          <p>학생의 마음 건강보다 성적표와 순위를 앞세우는 태도를 규탄한다.</p>
        </article>
        <article class="stat-card">
          <b>06</b>
          <p>학교가 보호의 공간이 아니라 경쟁 적응을 요구하는 공간으로 변해 가는 현실을 규탄한다.</p>
        </article>
      </div>
    </section>
      ...
      <form class="panel protest-form" id="postForm">
        <div class="form-grid">
          <div class="field">
            ...
            <input
              ...
              maxlength="80"
              placeholder="예: 진도만 남고 배움은 사라진 교실"
              required
            />
          </div>
          ...
          <div class="field">
            ...
            <select id="category" name="category">
              ${categories
                .map((item) => `<option ${item === category ? "selected" : ""}>${item}</option>`)
                .join("")}
            </select>
          </div>
        </div>
        ...
        <div class="field">
          ...
          <textarea
            ...
            name="body"
            maxlength="1200"
            placeholder="불편했던 구조, 강요된 경쟁, 사교육 압박, 주입식 수업, 체험학습 위축 등을 구체적으로 기록해 주세요."
            required
        ...
        <div id="notice" class="notice ${blockedUntil || state.message ? "show" : ""} ${state.messageType}">
          ${escapeHtml(blockedUntil ? blockText(blockedUntil) : state.message)}
        </div>
      </form>
    </section>
...
function renderPosts(posts, featuredOnly) {
  if (!posts.length) {
    return `<div class="empty">아직 등록된 의견이 없습니다.</div>`;
  }
      ...
      (post) => `
        <article class="post-card">
          <div class="post-top">
            ...
            ${post.featured ? `<span class="badge badge-dark">선정 의견</span>` : ""}
          </div>
          ...
          <div class="meta">
            ...
            ${post.updatedAt && post.updatedAt !== post.createdAt ? ` · 수정 ${formatDate(post.updatedAt)}` : ""}
          </div>
...
async function submitPost(event) {
  ...
  try {
    if (state.editing) {
      await api(`/api/posts/${state.editing.id}`, {
        method: "PUT",
        ...
        <section class="site-shell login-box">
          <form class="admin-card" id="adminLoginForm">
            ...
            <div class="field">
              ...
              <input id="adminPw" name="code" type="password" autocomplete="current-password" required />
            </div>

            <button class="btn" type="submit" id="adminLoginButton">로그인</button>

            <div id="notice" class="notice ${state.message ? "show" : ""} ${state.messageType}">
        ...
        <article class="admin-card">
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${state.posts.map(adminPostRow).join("") || `<tr><td colspan="5">등록 글 없음</td></tr>`}
              </tbody>
            </table>
          </div>
        </article>
        ...
        <article class="admin-card">
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${state.blocks.map(blockRow).join("") || `<tr><td colspan="5">차단된 IP 없음</td></tr>`}
              </tbody>
            </table>
          </div>
        </article>
  ...
  return `
    <tr>
      ...
      <td>
        ...
        <div class="help">
          ${escapeHtml(post.body).slice(0, 110)}${post.body.length > 110 ? "…" : ""}
        </div>
      </td>
      ...
      <td>
        <div class="admin-row-actions">
          <button
            class="btn small ${post.featured ? "secondary" : "ok"}"
            data-action="feature"
            data-id="${post.id}"
            data-featured="${post.featured ? "false" : "true"}"
          >
            ${post.featured ? "선정 해제" : "선정"}
          </button>
          <button class="btn danger small" data-action="admin-delete" data-id="${post.id}">
            삭제
          </button>
        </div>
      </td>
...
function blockRow(block) {
  return `
    <tr>
      <td>
        <b>${escapeHtml(block.ip)}</b>
        <div class="help">${escapeHtml(block.maskedIp || "")}</div>
      </td>
      <td>${escapeHtml(block.reason || "")}</td>
      <td>${formatDate(block.blockedAt)}</td>
      <td>${formatDate(block.blockedUntil)}</td>
      <td>
        <button class="btn secondary small" data-action="unblock" data-ip="${escapeHtml(block.ip)}">
          해제
        </button>
      </td>
    </tr>
...
async function loadAdmin() {
  ...
  try {
    const blocksData = await api("/api/admin/blocks", {
      headers,
    });
...
async function adminLoginFromForm(formEl) {
  if (!(formEl instanceof HTMLFormElement)) {
    setMessage("폼을 찾지 못했습니다. 페이지를 새로고침해 주세요.", "error");
    return;
  }
...
async function adminLogin(event) {
  event.preventDefault();
  await adminLoginFromForm(event.target);
}
...
async function handleClick(event) {
  const loginButton = event.target.closest?.(
    "#adminLoginForm button[type='submit'], #adminLoginForm button:not([type]), #adminLoginButton"
  );
  ...
  if (action === "unblock") {
    try {
      await adminAction("/api/admin/blocks", {
